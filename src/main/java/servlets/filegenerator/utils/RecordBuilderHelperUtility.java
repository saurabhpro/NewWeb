...
public class RecordBuilderHelperUtility {
	...
	public static void buildSingleRecord(String id, String fileToUse, String excelOrPdf, String temporaryFilePath,
		...
		if (excelOrPdf.equals("PDF")) {
			...
			CreateSingleRecordPDF.createPDFFromJson(temporaryFilePath + ProjectConstants.FILE_SEPARATOR + pdfFileName, id, fileToUse);
			baos = convertFileToByteArrayOutputStream(temporaryFilePath + ProjectConstants.FILE_SEPARATOR + pdfFileName);
			...
			CreateSingleRecordExcel.createExcelFromJson(temporaryFilePath + ProjectConstants.FILE_SEPARATOR + excelFileName, id, fileToUse);
			baos = convertFileToByteArrayOutputStream(temporaryFilePath + ProjectConstants.FILE_SEPARATOR + excelFileName);
		}
	...
	public static void buildMultipleRecords(List<String> listOfIds, String fileToUse, String excelOrPdf,
		...
		if (excelOrPdf.equals("DOWNLOAD_PDF")) {
			...
			CreateMultipleRecordPDF.createPDF(temporaryFilePath + ProjectConstants.FILE_SEPARATOR + pdfFileName, listOfIds, fileToUse);
			baos = convertFileToByteArrayOutputStream(temporaryFilePath + ProjectConstants.FILE_SEPARATOR + pdfFileName);
			...
			CreateMultiRecordExcel.fromJsonToExcel(temporaryFilePath + ProjectConstants.FILE_SEPARATOR + excelFileName, listOfIds, fileToUse);
			baos = convertFileToByteArrayOutputStream(temporaryFilePath + ProjectConstants.FILE_SEPARATOR + excelFileName);
		}
