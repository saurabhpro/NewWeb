...
class CombineFileHelperUtility {
    ...
    static void leaveCountAndHolidayUpdater() {
        ...
        BiometricFileWorker.empBiometricMap.values().forEach((empObj) -> CombineFileHelperUtility.holidayStatusUpdater(empObj));
    }

    /**
     * Method to process the Biometric File Object and update the unaccounted absence status to present or half-day with
     * Financial Force object information
     */
    static void unaccountedAbsentStatusUpdater() {
        ...
        //just replace CombineFileHelperUtility with this, if the method is npt static in this class
        BiometricFileWorker.empBiometricMap.values().forEach(CombineFileHelperUtility::setAbsentToUnaccountedAndHalfDayWorkTime);
    }
    ...
    private static void setNumberOfLeavesForEachEmployee(String bEmpID) {
        ...
        HrnetFileWorker.hrnetDetailsMap.keySet().stream().filter(hr -> hr.equals(finalSalesForce))
                .forEach(hr -> BiometricFileWorker.empBiometricMap
                        .get(bEmpID)
        ...
        for (int i = 0; i < ProjectConstants.getMONTH().maxLength(); i++) {
            ...
            switch (empObj.attendanceOfDate[i].getAttendanceStatusType()) {
                case ABSENT:
                    ...
                    Set<Map.Entry<String, ArrayList<EmployeeHrnetDetails>>> hrDataSet = HrnetFileWorker.hrnetDetailsMap.entrySet();
    ...
    private static void updateAbsentStatusWithLeaveToPresentOrHalfDay(EmployeeHrnetDetails hr, EmployeeBiometricDetails empObj) {
        ...
        do {
            ...
            if (leaveTime == 0.5) {
                empObj.attendanceOfDate[thisDate].setAttendanceStatusType(HALF_DAY);        //second point of update for Half_Day
