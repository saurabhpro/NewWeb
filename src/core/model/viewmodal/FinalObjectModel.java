 ...
 */
public class FinalObjectModel extends BasicEmployeeDetails implements Serializable {
	/**
	 *
	 */
	...
	public FinalObjectModel(String EmployeeID, int numberOfEntriesInHrNet, AttendanceOfDate[] a,
	                        ArrayList<EmployeeHrnetDetails> hr1) {
		this.setEmpId(EmployeeID);

		BasicEmployeeDetails b = InitialObjects.allEmployeeBasicRecordMap.get(EmployeeID);
	...
	public void displayFinalList() {
		...
		for (int j = 0; j < getNumberOfDaysConsideredInRespectiveMonth(); j++) {
			...
			System.out.print(
					"\t" + EMP_WORK_HOURS_FOR_THIS_DAY + " : " + this.attendanceOfDate[j].getWorkTimeForDay() + "\n");
		}
